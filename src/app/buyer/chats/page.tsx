...
type BuyerFilter = 'all' | 'replied' | 'with_price' | 'in_progress'
...
function matchesFilter(conv: RequestConversation, f: BuyerFilter): boolean {
  if (f === 'all')         return true
  if (f === 'replied')     return conv.hasSellerReply
  if (f === 'with_price')  return conv.hasPriceSent
  if (f === 'in_progress') return conv.status === 'in_progress'
  return true
}
...
function ConversationCard({ conv }: { conv: RequestConversation }) {
  const { label: statusLabel, variant: statusVariant } = BUYER_STATUS_CONFIG[conv.status]

  return (
    <div className="bg-white border border-slate-200 rounded-xl px-4 py-4">
      {/* Top row */}
      <div className="flex items-start justify-between gap-2 mb-1.5">
        <span className="text-sm font-semibold text-slate-900 truncate">{conv.sellerName}</span>
      ...
      <div className="flex items-center gap-1.5 flex-wrap mb-3">
        ...
        {conv.hasPriceSent && (
          <Badge variant="success">Τιμή πωλητή</Badge>
        )}
        {conv.hasSellerReply && (
          <Badge variant="brand">Απάντηση πωλητή</Badge>
        )}
      </div>

      {/* CTA */}
      <Link
        href={ROUTES.BUYER.ORDER_DETAIL(conv.requestId)}
  ...
  return (
    <PageContainer narrow className="pb-24 lg:pb-10">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500">Συζητήσεις με πωλητές για τα αιτήματά σου.</p>
      </div>
