...
import { getCurrentBuyerId } from '@/lib/data/buyer'
import { getBuyerChats } from '@/lib/data/chats'
...
type Filter = 'all' | 'unread' | 'active' | 'closed'

const FILTER_OPTIONS: { value: Filter; label: string }[] = [
  { value: 'all', label: 'Όλα' },
  { value: 'unread', label: 'Αδιάβαστα' },
...
export default function BuyerChatsPage() {
  ...
  const allThreads = getBuyerChats(buyerId)
  const totalUnread = allThreads.reduce((sum, t) => sum + t.unreadCountBuyer, 0)

  const filtered = allThreads.filter((t: ChatThread) => {
    if (filter === 'unread' && t.unreadCountBuyer === 0) return false
    ...
    if (search.trim()) {
      ...
      const matchesSeller = t.sellerBusinessName.toLowerCase().includes(q)
      const matchesOrder = t.orderId ? formatOrderNumber(t.orderId).includes(q) : false
      if (!matchesPart && !matchesSeller && !matchesOrder) return false
    }
    return true
  })
    ...
    <div className="flex h-[calc(100dvh-7.5rem)] lg:h-[calc(100dvh-3.5rem)] overflow-hidden">
      ...
      <div className={cn(
        ...
        <div className="px-4 pt-4 pb-3 border-b border-slate-200 bg-white flex-shrink-0">
          <div className="mb-3">
            <h1 className="text-lg font-semibold text-slate-900">Μηνύματα</h1>
            {totalUnread > 0 && (
              <p className="text-xs text-slate-500 mt-0.5">{totalUnread} αδιάβαστα</p>
            )}
          </div>
          <SearchInput
            placeholder="Ανταλλακτικό, πωλητής, παραγγελία..."
            value={search}
